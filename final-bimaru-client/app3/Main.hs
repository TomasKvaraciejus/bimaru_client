...
import Lib2 ( renderDocument, ToDocument, toDocument )
import Lib3 ( parseDocument, gameStart, GameStart, GameStart(..))
import Types(Check(..), Check, Coord, toDocument, fromDocument, fDocument)
import Network.Wreq
...
cmd c
  ...
  | commandToggle `L.isPrefixOf` trim c = do
    case tokens c of
      ...
      t -> lift $ modify (\((u, s), i) -> ((u, Lib1.toggle s (L.drop 1 t)), i))
...
save ((url, State _ _ t), i) = do
  ...
  liftIO $ Prelude.putStrLn $ "Game of id #" ++ show i ++ " saved successfully!" 
...
ini = do
  ((url, s), i) <- lift get
  -- liftIO $ TIO.putStrLn (pack url)
  let url' = if i == 0 then url ++ "/add" else url ++ "/load"
  r <- liftIO $ Wreq.get url'
  -- liftIO $ print (r ^. responseBody)
  let gs = (Lib3.parseDocument (cs (r ^. responseBody)) >>= fromDocument) :: ExceptT String IO Lib3.GameStart
  gs' <- liftIO $ runExceptT gs
  case gs' of
    Left msg -> liftIO $ fatal $ cs msg
    (Right (GameStart a b c)) -> do
      let gameId = if url' == url ++ "/add" then a else i
      lift $ put ((url, Lib3.gameStart s (GameStart gameId b c)), gameId)
      liftIO $ TIO.putStrLn $ pack $ "Your game id: #" ++ show gameId
...
run token = do
  ...
  let fullUrl = T.unpack "http://localhost:4200/"
  let loadUrl = if num == 0 then T.unpack "http://localhost:4200" else T.unpack "http://localhost:4200/load/" ++ show num
...
getStartingState _ url = do
  r <- liftIO $ Wreq.get url
  let gs = (Lib3.parseDocument (cs (r ^. responseBody)) >>= fDocument) :: ExceptT String IO [Coord]
  gs' <- liftIO $ runExceptT gs
  case gs' of
    Left msg -> do
      return Lib1.emptyState
    Right d -> do
      case Lib1.emptyState of
        (State a b _) -> return $ State a b d
        _ -> return Lib1.emptyState

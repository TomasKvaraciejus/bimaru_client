...
module Lib2 (toDocument, ToDocument, renderDocument, gameStart, isDocumentCorrect, wrongState, keysFound, notFoundKey, checkNumHints, toMap, makeList, findSubstring, checkHintLength, fillUpHints) where
import Lib1 (State(..))
import Types
import Text.Read
...
gameStart (State b c d) e =
    if null b && null c && null d && isDocumentCorrect e
    then moveWhenStateCorrect (State b c d) e
...
continueGameStart (State _ _ d) e = do

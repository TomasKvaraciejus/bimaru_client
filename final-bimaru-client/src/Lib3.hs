...
module Lib3(gameStart, parseDocument, GameStart, GameStart(..)) where
...
parseDocument :: String -> ExceptT String IO Document
...
createType :: String -> ExceptT String IO Document -- creates base type (DString, DInteger, DNull)
createType str
...
createDocument :: String -> ExceptT String IO Document
createDocument str
...
createDList :: String -> Int -> ExceptT String IO [Document]
...
createDListElement :: String -> ExceptT String IO Document
createDListElement = createDocument

createDMap :: String -> Int -> ExceptT String IO [(String, Document)] -- recursively creates DMap
createDMap [] _ = return []
createDMap str n =
    do
...
parseDMap :: String -> ExceptT String IO (String, Document)
parseDMap str = do
    key    <- checkDMapKey $ readLine str
    value  <- parseDMapValue str
    return (key, value)

parseDInteger :: String -> ExceptT String IO Document
parseDInteger str
    | elem '-' str =
...
parseDMapValue :: String -> ExceptT String IO Document
parseDMapValue str 

...
parseDocument str = if take 1 (reverse str) == "\n" then createDocument str else Left $ "missing endl at end"
...
createType str
    ...
    | removeForwardSpacing (readLine str) == "null" || removeForwardSpacing (readLine str) == "~" = Right DNull
...
dropUntilSameSpacing str n dmap
    ...
    | otherwise = if (spacingLength (str)) == n && (elem '-' (readLine str)) then Right str 
                  else 
                      if (spacingLength (str)) < n then Right [] 
                      else dropUntilSameSpacing (dropUntil str '\n') n dmap

...
renderDocument (DList l)    = goDeeper l
renderDocument _     = wrongInput

-- First level of Document validation - is the document list correct
goDeeper :: [Document] -> String
goDeeper l = do
    let isCorrect = listCorrect l 0
    case isCorrect of
        Right _ -> "{coords: [" ++ parseList l ++ "]}"
        Left str -> str


-- List can be a set of Documents.  
listCorrect :: [Document] -> Int -> Either String String
listCorrect [] _ = Right "OK"
listCorrect [(DMap [x,y])] _ = do
    case ifActuallyCorrect x y [] of
        Right _ -> Right "OK"
        Left str -> Left str
listCorrect ((DMap [x, y]):xs) i = do
    case ifActuallyCorrect x y xs of
        Right z -> listCorrect z (i + 1)
        Left str -> Left str
listCorrect ((DMap [_]):_)   _ = Left "Not enough arguments in one of DMaps"
listCorrect ((DMap []):_)    _ = Left "Not enough arguments in one of DMaps"
listCorrect (_:_) i = if i > 0 then Left incorrectDMap else Left wrongInput


ifActuallyCorrect :: (String, Document) -> (String, Document) -> [Document] -> Either String [Document]
ifActuallyCorrect (x1, DInteger y1) (x2, DInteger y2) list =
    if x1 == "col: " && x2 == "row: " && y1 >= 0 && y1 <=9 && y2 >= 0 && y2 <= 9
    then Right list
    else Left "Found incorrect DMap in DList"
ifActuallyCorrect _ _ _ = Left "Found incorrect DMap in DList"


parseList :: [Document] -> String
parseList (DMap (x:y:[]):[]) = "{" ++ fst x ++ show (toNum' $ snd x) ++ ", " ++ fst y ++ show (toNum' $ snd y) ++ "}"
parseList ((DMap [x, y]):xs) = "{" ++ fst x ++ show (toNum' $ snd x) ++ ", " ++ fst y ++ show (toNum' $ snd y) ++ "}, " ++ parseList xs
parseList _ = ""

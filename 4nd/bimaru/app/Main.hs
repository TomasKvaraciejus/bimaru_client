...
import Lib
import Helper (parseDocument, renderDocument, toDocument, fromDocument, fDocument)
import Types(Check(..), Check, Coord, State(..))
import Network.Wreq
...
type Repl a = HaskelineT (StateT ((String, Types.State), Int) IO) a
...
cmd :: String -> Repl ()
cmd c
  | trim c == commandShow = lift get >>= liftIO . Prelude.putStrLn . Lib.render . snd . fst
  | trim c == commandCheck = lift get >>= check . (Lib.mkCheck . snd . fst) >>= liftIO . Prelude.putStrLn
  ...
  | commandToggle `L.isPrefixOf` trim c = do
    case tokens c of
      ...
      t -> if even $ length t
           ...
           else lift $ modify (\((u, s), i) -> ((u, Lib.toggle s (L.drop 1 t)), i))
...
save :: ((String, Types.State), Int) -> Repl ()
save ((url, State _ _ t), i) = do
  let coordDList = Helper.toDocument $ Check t
  r <- liftIO $ Wreq.post (url ++ "/save/" ++ show i) (packChars $ Helper.renderDocument coordDList)
...
ini = do
  ...
  let h = L.evalState $ runExceptT Helper.parseDocument
  let h' = h (cs (r ^. responseBody))
  let gs = (except h' >>= fromDocument) :: ExceptT String IO Lib.GameStart
  gs' <- liftIO $ runExceptT gs
  case gs' of
    Left msg -> liftIO $ fatal $ cs msg
    (Right (GameStart a b c)) -> do
      let gId = if url' == url ++ "/add" then a else i
      lift $ put ((url, Lib.gameStart s (GameStart gId b c)), gId)
...
run :: String -> IO ()
run token = do
  let num = Main.getNum $ readMaybe token
...
getStartingState :: Int -> String -> IO Types.State
getStartingState 0 _ = return Lib.emptyState
getStartingState _ url = do
  ...
  gs' <- liftIO $ runExceptT gs
  case gs' of
    Left _ -> return Lib.emptyState
    Right d -> do
      case Lib.emptyState of
